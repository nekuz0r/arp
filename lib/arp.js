...
module.exports.getMACIPv6 = function (ipaddress, cb) {
	if (process.platform.indexOf('linux') == 0) {
		exports.readMACLinuxIPv6(ipaddress, cb);
	} else {
		cb(true, "No IPv6 support for current platform: " + process.platform);
	}
};

/**
 * read from ip -6 neighbor show
 */
module.exports.readMACLinuxIPv6 = function (ipaddress, cb) {
	...
	ping.on('close', function (code) {
		...
		var arp = spawn("ip", ['-6', 'neighbor', 'show']);
		var buffer = '';
		var errstream = '';
		...
		arp.on('close', function (code) {
			...
			for (let row of table) {
				...
				if (splittedRow[0] == ipaddress) {
					cb(false, splittedRow[4]);
					return;
				}
			}
